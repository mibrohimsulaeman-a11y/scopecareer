...
TARGET=SCRATCH/'scopecareer-web-cdp.json'
...
def browser_session():
    ...
    profile=Path(tempfile.mkdtemp(prefix='scopecareer-web-',dir=SCRATCH))
    server=subprocess.Popen(['python3','-m','http.server',str(http_port),'--bind','127.0.0.1'],cwd=ROOT,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    origin=f'http://127.0.0.1:{http_port}'; url=origin+(os.environ.get('SCOPECAREER_PROBE_URL') or '/web/?selfcheck=1')
    ...
    try:
        ...
        for _ in range(300):
            try:
                ...
                target=next((x for x in rows if x.get('type')=='page' and '/web/' in x.get('url','')),None)
                if target: break
