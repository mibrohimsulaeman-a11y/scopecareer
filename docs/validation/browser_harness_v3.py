...
def browser_session():
    ...
    server=subprocess.Popen(['python3','-m','http.server',str(http_port),'--bind','127.0.0.1'],cwd=ROOT/'validation/prototype-v3',stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def main():
    ...
    with browser_session():
        result=subprocess.run(['node','validation/prototype_smoke_v3_cdp.mjs'],cwd=ROOT,text=True,capture_output=True,env=env)
