...
const opened=await evalv(`({
  ...
  sections:['Case for','Watch','Route in','Sources'].map(t=>document.getElementById('oppDetail').textContent.includes(t)),
...
const checks={
  ...
  contextualCtaFollowsMissingWork:meridianCta==='Research unknowns'&&northstarCta==='Prepare for call',
  initialCtaClear:opened.actions.includes('Research unknowns')&&opened.actions.includes('Save for later')&&opened.actions.includes('Dismiss'),
  researchProgressive:researching.state==='Researching'&&researching.researchOpen===true&&researching.hasRoutes>=2,
  ctaMapsToCanonicalExplore:canonicalExplore?.canonical_action==='ACT-OPPORTUNITY-EXPLORE'&&canonicalExplore?.surface_label==='Research unknowns',
  pursuingCtaClear:researching.actions.includes('Start pursuing'),
  ...
  pursuingActionsAligned:final.state==='In progress'&&final.actions.includes('Open workspace')&&final.actions.includes('Add note')&&!final.actions.includes('Research further'),
